// ORM 4 postgres knex customer -- CP
...
const create = (customer) => {
    ...
    knex('orm4_customer').insert({id: `${customer.id}`, firstname: `${customer.firstName}`, middlename: `${customer.middleName}`, lastname: `${customer.lastName}`, dob: `${customer.dob}`, phone:`${customer.phone}`, email: `${customer.email}`, streetaddress: `${customer.streetAddress}`, city: `${customer.city}`, state: `${customer.state}`, zip: `${customer.zip}`, company: `${customer.company}`})
    .then((rows) => {
        // for (row of rows) {
        //     console.log(row)
        // }
...
const read = (id) => {
    ...
    knex
    .from('orm4_customer').select('*').where({'id': id})
    .then((rows) => {
    for (row of rows) {
...
const remove = (id) => {
    ...
    knex("orm4_customer").where("id",`${id}`).del().then(function (count) {
