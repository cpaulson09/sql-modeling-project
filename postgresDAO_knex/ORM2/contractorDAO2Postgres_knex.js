// ORM 2 postgres knex contractor -- CP
...
const create = (contractor) => {
    ...
    knex('orm2_contractor').insert({id: `${contractor.id}`, firstname: `${contractor.firstName}`, middlename: `${contractor.middleName}`, lastname: `${contractor.lastName}`, dob: `${contractor.dob}`, phone:`${contractor.phone}`, email: `${contractor.email}`, streetaddress: `${contractor.streetAddress}`, city: `${contractor.city}`, state: `${contractor.state}`, zip: `${contractor.zip}`, company: `${contractor.company}`})
    .then((rows) => {
        // for (row of rows) {
        //     console.log(row)
        // }
...
const read = (id) => {
    ...
    knex
    .from('orm2_contractor').select('*').where({'id': id})
    .then((rows) => {
    for (row of rows) {
...
const remove = (id) => {
    ...
    knex("orm2_contractor").where("id",`${id}`).del().then(function (count) {
