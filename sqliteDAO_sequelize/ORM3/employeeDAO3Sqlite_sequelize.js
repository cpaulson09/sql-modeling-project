...
const read = async (id) => {
  const employee = await Person.findAll({
    where: {
      firstName: 'James'
    }
  })
...
const update = async (executive) => {
  const employee = await Person.update({ company: 'and Jesus x2' }, {
    where: {
      firstName:'James' 
    }
  });
...
const remove = async (id) => {
  const employee = await Person.destroy({
    where: {
      lastName: 'Smith'
    }
  });
